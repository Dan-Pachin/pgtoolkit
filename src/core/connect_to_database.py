...
def connect_to_database(database_input="postgres", user_input="postgres"):
    try: 
        connection = psycopg2.connect(dbname=database_input, user=user_input, host="127.0.0.1") 
        ...
        connection = psycopg2.connect(dbname=database_input, user=user_input, password = password_input, host="127.0.0.1") 
        connection.set_isolation_level(psycopg2.extensions.ISOLATION_LEVEL_AUTOCOMMIT)
